...
#
#         self.sa0 = RIConv2SetAbstraction(npoint=1024, radius=0.12, nsample=16, in_channel=0 + in_channel,
#                                          mlp=[self.encoder_channel],
#                                          group_all=False)
#
...
#
#     # 对掩码进行排序，确保最接近的点在前面
#     dists[~mask] = float('inf')  # 将半径外的点距离设置为无穷大，以便后续排序
#     sorted_dists, sorted_idx = torch.sort(dists, dim=-1)  # [B, S, N]
#
#     # 选择前 nsample 个点
#     idx = sorted_idx[:, :, :nsample]  # [B, S, nsample]
#
#     # 计算每个区域内的点数
#     pts_cnt = mask.sum(dim=-1)  # [B, S]
#
#     # 如果某个区域内的点少于 nsample 个，则用第一个点进行填充
#     idx_expanded = idx.clone()
#     for b in range(B):
#         for s in range(S):
#             if pts_cnt[b, s] < nsample:
#                 idx_expanded[b, s, pts_cnt[b, s]:] = idx[b, s, 0]
#
#     return idx_expanded, pts_cnt

# def global_pca_adjust(eigen, x, mode='nearest_v1'):
#     # x: [B, S, 3]
#     # eigen: [B, 3, 3]
#     arr = []
#     diff = []
...
#
#         mask_neg = (det < 1e-6).float()  # invalid rotation
#         mask_det_neg = (mask_neg.view(*det.size(), 1, 1).expa
#         mask_det_pos = arr - mask_det_neg
#         arr = mask_det_neg @ I + mask_det_pos
#
#     return arr

# def PCA(xyz, adjust=False, mode='nearest_v1'):
#     """
#     :param xyz: [B, N, 3]
#     :param adjust:
#     :param mode:
...
#     basis = torch.linalg.eigh(sample_cov.cpu(), UPLO='U')[1]  # Use UPLO='U' for upper triangular
...
def RI_features(xyz, centers, center_norms, idx, pts, norm_ori):
    ...
    #                      grouped_xyz_angle_0,
    #                      grouped_xyz_angle_1,
    #                      grouped_xyz_angle_norm], dim=-1)
    #                         # grouped_xyz_inner_angle_0,
    #                         # grouped_xyz_inner_angle_1,
    #                         # grouped_xyz_inner_angle_2], dim=-1)

    # B, N, K = idx_ordered.shape
    # # sorting_indices = torch.zeros_like(idx_ordered)
    #
    # idx1_flat1 = idx_ordered.view(B, N, 1, K).expand(-1, -1, K, -1)
    # idx1_flat2 = idx.view(B, N, K, 1).expand(-1, -1, -1, K)
    #
    # # 计算排序索引
    # sorting_indices = (idx1_flat1 == idx1_flat2).nonzero()[:, -1].view(B, N, K)
    #
    # B, N, K, C = ri_feat.shape
    # ri_feat = torch.gather(ri_feat, 2, sorting_indices.unsqueeze(-1).expand(B, N, K, C))
